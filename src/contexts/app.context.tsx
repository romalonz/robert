...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    // Robert must never launch at login; it runs only when Robert.command is
    // clicked. Force autostart OFF on every launch so it can never come back.
    const ensureAutostartOff = async () => {
      try {
        await disable();
      } catch (error) {
        console.debug("Autostart disable skipped:", error);
      }
    };
    ensureAutostartOff();
  }, []);
