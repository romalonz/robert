...
pub fn set_window_height(window: tauri::WebviewWindow, height: u32) -> Result<(), String> {
    ...
    let new_size = LogicalSize::new(720.0, height as f64);
    window
...
/// Creates the Robert window: content-protected (off screen shares) and on top,
/// pointing at the /robert route.
pub fn create_robert_window<R: Runtime>(
    app: &AppHandle<R>,
) -> Result<WebviewWindow<R>, tauri::Error> {
    // Horizontal rectangle: a wide, short bar. Borderless and transparent, dragged
    // by the in-app top bar (data-tauri-drag-region). Resizable to adjust.
    let base_builder =
        WebviewWindowBuilder::new(app, "robert", tauri::WebviewUrl::App("/robert".into()))
            .title("Robert")
    ...
    let window = base_builder.build()?;
    // Hide on close instead of destroying, same as the dashboard.
    setup_dashboard_close_handler(&window);
    Ok(window)
}

#[tauri::command]
pub fn open_robert(app: tauri::AppHandle) -> Result<(), String> {
    if let Some(w) = app.get_webview_window("robert") {
        w.show().map_err(|e| format!("Failed to show robert window: {}", e))?;
        w.set_focus()
            .map_err(|e| format!("Failed to focus robert window: {}", e))?;
    } else {
        let w = create_robert_window(&app)
            .map_err(|e| format!("Failed to create robert window: {}", e))?;
        w.show()
            .map_err(|e| format!("Failed to show new robert window: {}", e))?;
        w.set_focus()
            .map_err(|e| format!("Failed to focus new robert window: {}", e))?;
    }
    Ok(())
}
